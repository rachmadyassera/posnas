...
<header>
    <table cellpadding="0" cellspacing="0" class="center">
        <thead>
            ...
            <tr>
                <td style="vertical-align: middle; " class="text-center"><h3>{{ $rapat->organization->name }}</h3></td>
            </tr>
            <tr>
    ...
    <table>
        <thead>
            ...
            <tr>
                ...
                <td style="vertical-align: top; " class="text-center"> : </td>
                <td style="vertical-align: top; " class="text-left"> {{ $carbon::parse($rapat->date_confrence)->isoFormat('dddd, D MMMM Y, H:m')  }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td style="vertical-align: top; " class="text-center"> : </td>
                <td style="vertical-align: top; " class="text-left"> {{ $peserta->count() }} Peserta (Laki-laki : {{ $peserta->where('gender','Laki-laki')->count() }}, Perempuan : {{ $peserta->where('gender','Perempuan')->count() }})</td>
            </tr>
        </thead>
    </table>
<br>
...
<div class="table-responsive">
    ...
    border-color: #000;" class="table table-bordered ">
        ...
        <tr>
            ...
            <td style="vertical-align: middle; border-width: 1px;
            border-color: #000;">{{ $pst->position }}</td>
            <td style="vertical-align: middle; border-width: 1px;
            border-color: #000;">{{ $pst->id_employee }}</td>
            <td style="vertical-align: middle; border-width: 1px;
            border-color: #000;">{{ $pst->organization }}</td>
            <td style="vertical-align: middle; border-width: 1px;
            border-color: #000;">{{ $pst->nohp }}</td>
            <td style="vertical-align: middle; border-width: 1px;
            border-color: #000;"><center>
                <img  style="width: 75%" src="{{ public_path('presensi/signature/'.$pst->signature) }}" alt="">
                </center></td>
        </tr>
        @endforeach
        </tbody>
    ...
    <footer class="text-right">
        <p class="pr-3 m-0 mb-2">
            ...
            Daftar kehadiran ini secara otomatis dibuat dari sistem informasi presensi kegiatan Pemerintah Kota Tanjungbalai
        </p>
