...
@section('content')
    ...
    <div class="col-12 mb-4">
        <div class="hero bg-info text-white shadow">
            <div class="hero-inner">
                ...
                <p class="lead">{{ $carbon::parse($kegiatan->date_confrence)->isoFormat('dddd, D MMMM Y') }}
                    <br>{{ $kegiatan->location->nama }}</p>
                <p class="lead">
                <h5>Jumlah Kehadiran : {{ $presence->count() }} Peserta</h5>
                </p>
                <p class="lead">
                <h5>Laki-laki : {{ $presence->where('gender', 'Laki-laki')->count() }}, Perempuan :
                    {{ $presence->where('gender', 'Perempuan')->count() }}</h5>
                </p>
            </div>
        </div>
    </div>
    <div class="card shadow">
        <div class="card-header">
            <h4 class="card-title">Data Presensi Peserta </h4>
            <div class="card-header-action">
                <div class="buttons">
                    <a href="{{ route('confrence.generatepdf', $kegiatan->id) }}" class="btn btn-icon btn-success"><i
                        ...
                        @foreach ($presence as $pre)
                            <tr>
                                ...
                                <td style="vertical-align: middle;" class="text-center">
                                    <a href="/presence/disable/{{ $pre->id }}" class="btn-sm btn-danger"
